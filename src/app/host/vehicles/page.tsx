...
export default function HostVehiclesPage() {
  ...
  async function handleDelete(id: string) {
    if (!confirm(t('host.confirmDeleteVehicle'))) return;
    ...
    <FadeIn>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold text-gray-900">{t('host.vehicles')}</h1>
        ...
        {listings.length === 0 && (
          <Alert variant="warning">{t('host.needListingForVehicle')}</Alert>
        )}
        ...
        {vehicles.length === 0 ? (
          <Card className="p-12 text-center">
            ...
            <h3 className="text-lg font-semibold text-gray-900 mb-2">{t('host.noVehiclesTitle')}</h3>
            <p className="text-gray-500 mb-6">{t('host.noVehiclesDesc')}</p>
            {listings.length > 0 && (
              <Button onClick={openCreate}>{t('host.addFirstVehicle')}</Button>
            )}
          </Card>
        ) : (
              ...
              <Card key={vehicle.id} className="p-5">
                <div className="flex items-start justify-between mb-3">
                  ...
                  <Badge variant={vehicle.active ? 'success' : 'gray'}>
                    {vehicle.active ? t('host.statusActive') : t('host.statusInactive')}
                  </Badge>
                </div>
                ...
                <div className="flex items-center gap-4 text-sm text-gray-500 mb-4">
                  <span className="flex items-center gap-1">
                    ...
                    {vehicle.capacity_passengers} {t('host.passengers')}
                  </span>
                  <span className="flex items-center gap-1">
                    <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" />
                    </svg>
                    {vehicle.capacity_luggage} {t('host.luggage')}
                  </span>
                ...
                <div className="flex items-center gap-2">
                  ...
                  >
                    {vehicle.active ? t('host.deactivate') : t('host.activate')}
                  </Button>
                  <Button
                    variant="ghost"
                    size="sm"
      ...
      <Modal
        ...
        title={editingVehicle ? t('host.editVehicle') : t('host.addVehicle')}
      >
        <div className="space-y-4">
          {error && <Alert variant="error">{error}</Alert>}
          <Input
            label={t('host.vehiclePlate')}
            value={form.plate}
          ...
          <div className="grid grid-cols-3 gap-3">
            <Input
              label={t('host.vehicleMake')}
              value={form.make}
              onChange={(e) => setForm({ ...form, make: e.target.value })}
              placeholder="Mercedes"
            />
            <Input
              label={t('host.vehicleModel')}
              value={form.model}
              onChange={(e) => setForm({ ...form, model: e.target.value })}
              placeholder="Sprinter"
            />
            <Input
              label={t('host.vehicleYear')}
              type="number"
              value={form.year}
          ...
          {!editingVehicle && listings.length > 0 && (
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">{t('host.assignToListing')}</label>
              <select
          ...
          <div className="flex gap-3 justify-end pt-2">
            ...
            <Button onClick={handleSave} loading={saving}>
              {editingVehicle ? t('common.save') : t('host.addVehicle')}
            </Button>
          </div>
        </div>
      </Modal>
